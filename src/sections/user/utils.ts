...
export function getComparator<Key extends keyof Member>(
  order: 'asc' | 'desc',
  orderBy: Key
): (
  a: Member,
  ...
  if (orderBy === "part") {
    return order === "desc"
      ? (a, b) => partComparator(a, b)
      : (a, b) => -partComparator(a, b);
  } else if (orderBy === "role") {
    return order === "desc"
      ? (a, b) => getRoleComparator(a, b)
      : (a, b) => -getRoleComparator(a, b);
  }
...
export function applyFilter({ inputData, comparator, filterName }: ApplyFilterProps) {
  ...
  if (filterName) {
    inputData = inputData.filter(
      (user) => user.name.toLowerCase().indexOf(filterName.toLowerCase()) !== -1 || user.nameKana.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }
