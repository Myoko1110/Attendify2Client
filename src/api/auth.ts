...
import type Part from '../abc/part';
...
export default class Auth {
  ...
  static async fromEmail(grade: Grade, part: Part, email: string): Promise<Member> {
    try {
      const result = await axios.post('/login/temp', { generation: grade.generation, part: part.value, email });
      return Member.fromSchema(MemberSchema.parse(result.data));
    } catch (e) {
      throw APIError.fromError(e);
    }
